...
trait Tables {
  ...
  implicit val applicationIdColumnType = MappedColumnType.base[ApplicationId, String](
    { aId =>
      ...
      ApplicationId(UUID.fromString(str))
    }
  )
  ...
  implicit def GetResultProfileRow(implicit e0: GR[ProfileId], e1: GR[ApplicationId], e2: GR[String]): GR[ProfileRow] =
    GR { prs =>
      import prs._
  ...
  class Profile(_tableTag: Tag) extends Table[ProfileRow](_tableTag, "profile") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? =
      (Rep.Some(profileId), Rep.Some(applicationId), Rep.Some(name)).shaped.<>({ r =>
        import r._; _1.map(_ => ProfileRow.tupled((_1.get, _2.get, _3.get)))
      }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val applicationFk = foreignKey("CONSTRAINT_ED", applicationId, Application)(
      r => r.applicationId,
      onUpdate = ForeignKeyAction.Restrict,
      onDelete = ForeignKeyAction.Restrict)
  }
  ...
  case class SchemaVersionRow(versionRank: Int,
                              installedRank: Int,
                              version: String,
                              description: String,
                              `type`: String,
  ...
  implicit def GetResultSchemaVersionRow(implicit e0: GR[Int],
                                         e1: GR[String],
                                         e2: GR[Option[Int]],
                                         e3: GR[java.sql.Timestamp],
                                         e4: GR[Boolean]): GR[SchemaVersionRow] = GR { prs =>
    import prs._
    SchemaVersionRow.tupled(
      (<<[Int],
       <<[Int],
       <<[String],
       <<[String],
  ...
  class SchemaVersion(_tableTag: Tag) extends Table[SchemaVersionRow](_tableTag, "schema_version") {
    def * =
      (versionRank,
       ...
       installedOn,
       executionTime,
       success) <> (SchemaVersionRow.tupled, SchemaVersionRow.unapply)

    /** Maps whole row to an option. Useful for outer joins. */
    def ? =
      (Rep.Some(versionRank),
       ...
       Rep.Some(success)).shaped.<>({ r =>
        import r._;
        _1.map(
          _ =>
            SchemaVersionRow.tupled(
              (_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7, _8.get, _9.get, _10.get, _11.get)))
      }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
